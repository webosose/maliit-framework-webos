...
public:
    ...
    InputMethodQuickPrivate(MAbstractInputMethodHost *host,
        ...
        , m_enterKeyType(MaliitQuick::DefaultEnterKeyType)
...
void InputMethodQuick::update()
{
    ...
    int newEnterKeyType = inputMethodHost()->enterKeyType(valid);
    if (!valid) {
        newEnterKeyType = MaliitQuick::DefaultEnterKeyType;
    }

    if (newEnterKeyType != d->m_enterKeyType) {
        d->m_enterKeyType = newEnterKeyType;
        emitEnterKeyType = true;
    }
...
int InputMethodQuick::enterKeyType()
{
    Q_D(InputMethodQuick);
    return d->m_enterKeyType;
}
